...
class BookServiceTest {
    ...
    void setUp() {
        ...
        given(bookRepository.findById(2L)).willReturn(Optional.of(new Book(2L, "123")));
         given(authorRepository.findAllByBooks(any())).willReturn(asList(new Author(1L, "1")));

        given(genreRepository.findAllByBooks(any())).willReturn(asList(new Genre(2L, "2")));
